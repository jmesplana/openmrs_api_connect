...
# New parser command for adding a location
...
parser_addlocation.add_argument('--parentLocation', '-pl', help='Parent location UUID')
parser_addlocation.set_defaults(func='addlocation')

#Add a new subparser so edit a location
...
parser_updatelocation.add_argument('--parentLocation', '-p', help='UUID of the new parent location')
...
# New function to create a location
...
def create_location(name, description=None, tags=None, country=None, parentLocation=None):
    url = '%s/location' % api
    data = {
        'name': name,
    ...
    if parentLocation:
        data['parentLocation'] = {'uuid': parentLocation}
    data_json = json.dumps(data)
    return openmrs_post(url, data_json)

#update a location
def update_location(uuid, name=None, description=None, tags=None, country=None, parentLocation=None):
    url = '%s/location/%s' % (api, uuid)
    data = {}
    if name:
        data['name'] = name
    if description:
    ...
    if tags:
        print(f"Tags before split: {tags}") # Logging
        try:
            tag_uuids = [get_tag_uuid(tag.strip()) for tag in tags.split(',')]
            data['tags'] = tag_uuids
        except Exception as e:
            print(f"Error processing tags: {e}") # Error handling
            sys.exit(1)
    if country:
        data['country'] = country
    if parentLocation:
    ...
    resp = requests.put(url, data_json, headers=headers, auth=(user, pw))
    try:
        resp_json = resp.json()
    except:
        print('Invalid response:')
        print(resp)
        sys.exit(1)
    return attr_from_response('uuid', resp_json)
...
def main():
	...
	#Add new location
	elif args.func == 'addlocation':
		location = create_location(
			name=args.name, 
			description=args.description,
			tags=args.tags,
			country=args.country,
			parentLocation=args.parentLocation
		)
		if not silence: print(location)

#edit a location
	elif args.func == 'updatelocation':
		updated_location = update_location(uuid=args.uuid, name=args.name, description=args.description, 
										tags=args.tags, country=args.country, parentLocation=args.parentLocation)
		if not silence: print(f"Location {args.uuid} updated successfully!")
